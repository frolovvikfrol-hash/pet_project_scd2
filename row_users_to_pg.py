# Генерирует пользователей и обновляет их данные в PostgreSQL
...
def add_new_user() -> None:
    ...
    first_date = fake.date_time_ad(
        start_datetime=date(year=2024, month=1, day=1),
        end_datetime=date(year=2025, month=1, day=1),
    )
    dict_ = {
        "id": uuid.uuid4(),
        ...
        "birthday": fake.date_time_ad(
            ...
            end_datetime=date(year=2005, month=1, day=1),
        ),
        "email": fake.email(),
        "ts_db": datetime.now(timezone.utc),
    }
    ...
    df.to_sql(
        schema="raw",
        name="raw_users",
        con="postgresql://postgres_1:postgres@localhost:5433/postgres",
        if_exists="append",
        index=False,
    )
...
def update_info_about_current_user() -> None:
    ...
    db_url = "postgresql://postgres_1:postgres@localhost:5433/postgres"
    ...
    try:
        ...
        new_user_data["ts_db"] = datetime.now(timezone.utc)
